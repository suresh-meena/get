...
from .energy import compute_energy_GET, compute_energy_and_grad_GET, compute_memory_entropy
...
class GETLayer(nn.Module):
    ...
    def compute_energy(self, X, batch_data, static_projections=None):
        ...
        return compute_energy_GET(X, G, batch_data.c_2, batch_data.u_2, batch_data.c_3, batch_data.u_3, batch_data.v_3, batch_data.t_tau, params, projections)
    ...
    def energy_and_grad(self, X, batch_data, static_projections=None, create_graph=False):
        if not hasattr(self.layernorm, "backward"):
            ...
            E = self.compute_energy(X, batch_data, static_projections); return E, torch.autograd.grad(E, X, create_graph=create_graph)[0]
        G = self.layernorm(X); projections = self._build_projections(G, static_projections, batch_data=batch_data)
        params = self.get_params_dict(); params['num_heads'] = self.num_heads
        E, grad_X_quad, head_grads = compute_energy_and_grad_GET(X, G, batch_data.c_2, batch_data.u_2, batch_data.c_3, batch_data.u_3, batch_data.v_3, batch_data.t_tau, params, projections)
        grad_G_att = torch.zeros_like(G)
        
        # Avoid einsum as a workaround for torch.compile TypeError bug in some versions
        if self.use_pairwise and self.W_Q2 is not None:
            gQ2, gK2 = head_grads.get('grad_Q2'), head_grads.get('grad_K2')
            if gQ2 is not None: grad_G_att += (gQ2 @ self.W_Q2).sum(dim=-3)
            if gK2 is not None: grad_G_att += (gK2 @ self.W_K2).sum(dim=-3)
        if self.use_motif and self.W_Q3 is not None:
            gQ3, gK3 = head_grads.get('grad_Q3'), head_grads.get('grad_K3')
            if gQ3 is not None: grad_G_att += (gQ3.flatten(-2, -1) @ self.W_Q3).sum(dim=-3)
            if gK3 is not None: grad_G_att += (gK3.flatten(-2, -1) @ self.W_K3).sum(dim=-3)
        if self.use_memory and self.W_Qm is not None:
            gQm = head_grads.get('grad_Qm')
            if gQm is not None: grad_G_att += (gQm @ self.W_Qm).sum(dim=-3)
...
class GETModel(nn.Module):
    def __init__(self, in_dim, d=256, num_classes=1, num_steps=8, compile=False, eta=0.05, eta_max=0.25, dropout=0.1, num_heads=1, head_dim=None, encoder_hidden_mult=2, readout_hidden_mult=2, pe_k=0, rwse_k=0, num_motif_types=2, use_cls_token=False, cls_self_loop=True, **layer_kwargs):
        super().__init__()
        self.d = d; self.num_steps = num_steps; self.eta_max = eta_max; self.pe_k = pe_k; self.rwse_k = rwse_k; self.use_cls_token = bool(use_cls_token); self.cls_self_loop = bool(cls_self_loop)
    ...
    def _run_fixed_solver(self, X, solver_batch, static_projections, training_mode=True):
        ...
        for _ in range(self.num_steps): X, E = self.get_layer(X, solver_batch, eta, static_projections, is_training=training_mode); energy_trace.append(E.detach())
    ...
    def _run_armijo_solver(self, X, batch_data, static_projections, armijo_c=0.1, armijo_gamma=0.5, armijo_eta0=0.2, armijo_max_backtracks=25):
        ...
        for _ in range(self.num_steps):
            ...
            grad_norm_sq = (grad_X**2).sum(dim=-1); etas = eta0 * (armijo_gamma ** torch.arange(armijo_max_backtracks, device=X.device, dtype=X.dtype))
            ...
            if E_tries.dim() == 1: E_tries = E_tries.unsqueeze(-1)
            E_target = E_t.detach().unsqueeze(0) - armijo_c * etas.view(-1, 1) * grad_norm_sq.unsqueeze(0).sum(dim=-1)
            valid = (E_tries <= E_target); found = valid.any(dim=0); best_idx = valid.to(torch.long).argmax(dim=0) if found.all() else E_tries.argmin(dim=0)
            if best_idx.dim() == 0: X = X_tries[best_idx]
            else: X = X_tries[best_idx, torch.arange(X.size(0))]
            energy_trace.append(E_t.detach()); stats['backtracks'].append(best_idx.float().mean().item()); stats['step_sizes'].append(etas[best_idx].mean().item()); stats['accepted'].append(found.float().mean().item())
