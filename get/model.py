...
from .energy import compute_energy_GET, compute_energy_and_grad_GET
...
class GETLayer(nn.Module):
    ...
    def energy_and_grad(self, X, batch_data, static_projections=None, create_graph=False):
        if not hasattr(self.layernorm, "backward"):
            ...
            return E, torch.autograd.grad(E, X, create_graph=create_graph)[0]
        G = self.layernorm(X)
        projections = self._build_projections(G, static_projections, batch_data=batch_data)
        params = self.get_params_dict()
        params['num_heads'] = self.num_heads
        ...
        if self.use_pairwise and self.W_Q2 is not None:
            ...
            if gQ2 is not None:
                grad_G_att += (gQ2 @ self.W_Q2).sum(dim=-3)
            if gK2 is not None:
                grad_G_att += (gK2 @ self.W_K2).sum(dim=-3)
        if self.use_motif and self.W_Q3 is not None:
            gQ3, gK3 = head_grads.get('grad_Q3'), head_grads.get('grad_K3')
            if gQ3 is not None:
                grad_G_att += (gQ3.flatten(-2, -1) @ self.W_Q3).sum(dim=-3)
            if gK3 is not None:
                grad_G_att += (gK3.flatten(-2, -1) @ self.W_K3).sum(dim=-3)
        if self.use_memory and self.W_Qm is not None:
            gQm = head_grads.get('grad_Qm')
            if gQm is not None:
                grad_G_att += (gQm @ self.W_Qm).sum(dim=-3)
...
class GETModel(nn.Module):
    ...
    def _run_armijo_solver(self, X, batch_data, static_projections, armijo_c=0.1, armijo_gamma=0.5, armijo_eta0=0.2, armijo_max_backtracks=25):
        ...
        for step_idx in range(self.num_steps):
            ...
            best_idx = valid.to(torch.long).argmax(dim=0) if found.all() else E_tries.argmin(dim=0)
            if best_idx.dim() == 0:
                X = X_tries[best_idx]
            else:
                X = X_tries[best_idx, torch.arange(X.size(0))]
