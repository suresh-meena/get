...
def segment_logsumexp(x, segment_ids, num_segments, return_intermediates=False):
    ...
    if return_intermediates: return lse, exp_x, denom
    return lse
...
def inverse_temperature(params, name, beta_max=None):
    ...
    if beta_max is not None:
        if torch.is_tensor(beta): beta = beta.clamp(max=beta_max)
        else: beta = min(beta, beta_max)
    return beta
...
def compute_pairwise_energy(G, c_2, u_2, params, projections, num_nodes, return_grad=False):
    d = params['d']; beta_max = params.get('beta_max', None)
    ...
    if (torch.is_tensor(lambda_2) and lambda_2 <= 1e-6) or (not torch.is_tensor(lambda_2) and lambda_2 <= 1e-6) or c_2.numel() == 0:
        return (G.new_zeros(G.shape[:-2]), (G.new_zeros(G.shape), G.new_zeros(G.shape))) if return_grad else G.new_zeros(G.shape[:-2])
    Q2, K2 = projections['Q2'], projections['K2']; scale = d ** 0.5
    ell_2 = _pairwise_dot_fused(Q2, K2, c_2, u_2, num_nodes, scale)
    if params.get("pairwise_symmetric", False): ell_2 = ell_2 + _pairwise_dot_fused(Q2, K2, u_2, c_2, num_nodes, scale)
    a_2 = projections.get('a_2')
    if a_2 is not None: ell_2 = ell_2 + a_2
    ...
    if not return_grad: return E
    probs = exp_x / denom[..., c_2]; coeff = lambda_2 * probs / scale
...
def compute_motif_energy(G, c_3, u_3, v_3, t_tau, params, projections, num_nodes, return_grad=False):
    d = params['d']; R = params.get('R', 1); beta_max = params.get('beta_max', None)
    ...
    if t_tau.numel() > 0 and t_tau.max() >= T_params.size(0): t_tau = torch.clamp(t_tau, max=T_params.size(0) - 1)
    T_tau_selected = T_params[t_tau].transpose(0, 1) # [H, L, R, d_h]
    scale = (R * d) ** 0.5; ell_3 = fused_motif_dot(Q3[..., c_3, :, :], K3[..., u_3, :, :], K3[..., v_3, :, :], T_tau_selected) / scale
    ...
    if not return_grad: return E
    probs = exp_x / denom[..., c_3]; coeff = (lambda_3 * probs / scale).unsqueeze(-1).unsqueeze(-1)
    src_Q = coeff * (K3[..., u_3, :, :] * K3[..., v_3, :, :] + T_tau_selected)
    src_Ku = coeff * (Q3[..., c_3, :, :] * K3[..., v_3, :, :]); src_Kv = coeff * (Q3[..., c_3, :, :] * K3[..., u_3, :, :])
...
def compute_memory_energy(G, params, projections, return_grad=False):
    ...
    scale = d ** 0.5; beta_m = inverse_temperature(params, 'beta_m', beta_max=beta_max)
    Lm = (Qm @ Km.transpose(-2, -1)) / scale; lse_m = torch.logsumexp(beta_m * Lm, dim=-1)
    E = (lambda_m / beta_m) * lse_m.sum(dim=-1)
    if not return_grad: return E
    probs = torch.softmax(beta_m * Lm, dim=-1); grad_Qm = (lambda_m / scale) * (probs @ Km)
    return E, (grad_Qm, None)

def compute_energy_GET(X, G, c_2, u_2, c_3, u_3, v_3, t_tau, params, projections=None):
    N = X.size(-2); E_quad = compute_quadratic_energy(X)
    ...
    if E_att2.dim() > E_quad.dim(): E_att2 = E_att2.mean(dim=-1)
    if E_att3.dim() > E_quad.dim(): E_att3 = E_att3.mean(dim=-1)
    if E_mem.dim() > E_quad.dim(): E_mem = E_mem.mean(dim=-1)
...
def compute_energy_and_grad_GET(X, G, c_2, u_2, c_3, u_3, v_3, t_tau, params, projections=None):
    ...
    grads = {'grad_Q2': gQ2/H, 'grad_K2': gK2/H, 'grad_Q3': gQ3/H, 'grad_K3': gK3/H, 'grad_Qm': (gQm/H) if gQm is not None else None}
...
def compute_memory_entropy(G, params, projections, eps=1e-12):
    if not params.get('use_memory', True) or params.get('K', 0) <= 0: return torch.zeros(G.shape[:-2], dtype=G.dtype, device=G.device)
    Qm, Km = projections['Qm'], projections['Km']; d = params['d']
    Lm = (Qm @ Km.transpose(-2, -1)) / (d ** 0.5); beta_m = inverse_temperature(params, 'beta_m', beta_max=params.get('beta_max', None))
    probs = torch.softmax(beta_m * Lm, dim=-1); entropy = -(probs * torch.log(probs.clamp_min(eps))).sum(dim=-1)
