...
def main():
    ...
    results = run_model_suite(
        ...
        test_ds,
        [
            {"name": "FullGET", "model": FullGET(1, 64, 1, num_steps=16, R=2, lambda_3=1.0, update_damping=0.1), "trainer_kwargs": {"lr": 1e-4}, "report_label": "Test MAE"},
            {"name": "ETFaithful", "model": ETFaithful(1, 64, 1, num_steps=16, mask_mode="sparse", et_official_mode=False), "trainer_kwargs": {"lr": 1e-4}, "report_label": "Test MAE"},
            {"name": "PairwiseGET", "model": PairwiseGET(1, 110, 1, num_steps=8), "trainer_kwargs": {"lr": 1e-4}, "report_label": "Test MAE"},
            {"name": "GIN", "model": GINBaseline(1, 64, 1), "trainer_kwargs": {"lr": 1e-4}, "report_label": "Test MAE"},
        ],
        device=device,
        epochs=args.epochs,
